...
def create_parser():
    ...
    parser.add_argument('--es_host', default='localhost', help='Hostname of elasticsearch, default'
                                                               ' = localhost')
    parser.add_argument('--es_port', type=int, default=9200, help='Hostname of elasticsearch, '
                                                                  'default = 9200')
    ...
    parser.add_argument('--dict_encoding', default='latin-1', help='Dictionary file encoding, '
                                                                   'default = latin-1')
...
def setup_es(host, port, es_index, delete_index):
    ...
    es_connection = Elasticsearch([{'host': host, 'port': port}])
    if delete_index:
        print 'Deleting index %s...' % es_index
        es_connection.indices.delete(es_index)
    # ignore 400 cause by IndexAlreadyExistsException when creating an index
    print 'Creating index %s...' % es_index
    es_connection.indices.create(index=index)
    ...
    with open(words_file) as dict_file:
        ...
        for line in dict_file:
            ...
            if len(es_actions) >= 500:
                ...
                helpers.bulk(es_conn, es_actions)
                es_actions = []
